...
def learn(config: Config, msg="default"):
    ...
    for i in range(n_epoch):
        ...
        sample_sxar, demo_sxar, sample_r, demo_r = sample_batch(il, sampling_agent, n_sample, demo_sa_array) # n_sample is too big
        ...
        if (i + 1) % config.log_interval == 0:
            ...
            print(f"{i}: r-sample-avg={sample_r}, r-demo-avg={demo_r}, log_p={v_l} ; {msg}")
        else:
            print(f"{i}: r-sample-avg={sample_r}, r-demo-avg={demo_r} ; {msg}")
...
if __name__ == "__main__":
    ...
    arg.add_arg("algo", "option_gail", "which algorithm to use, can be [gail, option_gail, DI_gail]")
    arg.add_arg("device", "cuda:0", "Computing device")
    arg.add_arg("tag", "default", "Experiment tag")
    arg.add_arg("seed", 0, "Random seed")

    arg.add_arg("n_pretrain_epoch", 50, "Pre-training epochs")
