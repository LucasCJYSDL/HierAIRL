...
class DistReward4Rooms(GoalReward4Rooms, DistRewardMixIn):
    ...
    def __init__(self, scale: float) -> None:
        ...
        self.mid_goals = [MazeGoal(np.array([-3.0 * scale, 6.0 * scale])),
...
class DistRewardCorridor(GoalRewardCorridor):
    ...
    def __init__(self, scale: float) -> None:
        super().__init__(scale)
        self.goals = [MazeGoal(np.array([-3.0 * scale, 5.0 * scale]))]
        self.mid_goals = [MazeGoal(np.array([0.0 * scale, -3.0 * scale])), MazeGoal(np.array([-5.0 * scale, -3.0 * scale])), MazeGoal(np.array([-5.0 * scale, 2.0 * scale])),
                          MazeGoal(np.array([0.0 * scale, 2.0 * scale])), MazeGoal(np.array([0.0 * scale, 7.0 * scale])),
                          MazeGoal(np.array([-3.0 * scale, 7.0 * scale])), MazeGoal(np.array([-3.0 * scale, 5.0 * scale]))]
    ...
    def expert_action(self, cur_obs):
        ...
        if self.goal_idx == 0 or self.goal_idx == 6:
            option = 0
        elif self.goal_idx == 1 or self.goal_idx == 5:
            option = 1
        elif self.goal_idx == 2 or self.goal_idx == 4:
            option = 2
        else:
            option = 3
