...
class MujocoEnv(object):
    ...
    def get_expert_act(self, obs):
        return self.env.get_expert_action(obs)
...
def collect_demo(config, n_demo=2048, display=False, is_manual=False, env_name=None, expert_path=None):
    ...
    # you must have a expert model first, by running 'option_ppo_learn.py'.
    ...
    while n_current_demo < n_demo:
        ...
        #     r_array = torch.as_tensor(r_array, dtype=torch.float32).unsqueeze(dim=1)
            with torch.no_grad():
                ...
                r_array = []
                s, done = env.reset(), False
                ct = torch.empty(1, 1, dtype=torch.long).fill_(policy.dim_c)
                c_array.append(ct)
                while not done:
                    st = torch.as_tensor(s, dtype=torch.float32, device=policy.device).unsqueeze(0)
                    ct = policy.sample_option(st, ct, fixed=True).detach()
                    at = policy.sample_action(st, ct, fixed=True).detach()
                    s_array.append(st)
                    c_array.append(ct)
                    a_array.append(at)
...
if __name__ == '__main__':
    ...
    # collect_demo(config=None, n_demo=80000, is_manual=True, env_name='Point4Rooms-v1')
    # get_demo_stat(path="/home/wenqi/Proj_2_HAIRL/Hier AIRL/data/mujoco/Point4Rooms-v1_sample.torch")
    # collect_demo(config=None, n_demo=80000, is_manual=True, env_name='PointCorridor-v1')
    # get_demo_stat(path="/home/wenqi/Proj_2_HAIRL/Hier AIRL/data/mujoco/PointCorridor-v1_sample.torch")
    # get_demo_stat(path="./Hopper-v2_sample.torch")
    # get_demo_stat(path="./Ant-v3_sample.torch")
    ...
    arg.add_arg("env_name", "Hopper-v2", "Environment name")
    ...
    collect_demo(config, n_demo=10000, expert_path='/home/wenqi/Proj_2_HAIRL/Hier AIRL/envir/model/hopper/1299.torch')
