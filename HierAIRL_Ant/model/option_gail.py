...
class GAIL(torch.nn.Module):
    def __init__(self, config: Config, dim_s=2, dim_a=2):
        ...
        self.optim = torch.optim.Adam(self.discriminator.parameters(), lr=lr, weight_decay=3.e-5)
        ...
        for _ in range(n_step):
            ...
            for ind_p in inds.split(self.mini_bs):
                ...
                for _ in range(1):
                    ...
                    loss += self.discriminator.gradient_penalty(s_array, a_array)
        ...
        for _ in range(n_step):
            ...
            for ind_p in inds.split(self.mini_bs):
                ...
                for _ in range(3):
                    ...
                    alpha = torch.rand(sp_b.size(0), 1, dtype=torch.float32, device=self.device)

                    inter_s = alpha * se_b + (1 - alpha) * sp_b
                    inter_a = alpha * ae_b + (1 - alpha) * ap_b

                    gradient_penalty = self.discriminator.gradient_penalty(inter_s, inter_a)
...
class OptionGAIL(torch.nn.Module):
    def __init__(self, config: Config, dim_s=2, dim_a=2):
        ...
        self.optim = torch.optim.Adam(self.discriminator.parameters(), weight_decay=3e-5) # 1e-3
        self.to(self.device)
